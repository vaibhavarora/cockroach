...
func MVCCGetProto(
	...
	value, _, _, mvccGetErr := MVCCGet(ctx, engine, key, timestamp, consistent, txn, nil, false)
	found := value != nil
...
func MVCCPutProto(
	...
	return MVCCPut(ctx, engine, ms, key, timestamp, value, txn, nil, false)
}
...
func MVCCGetAsTxn(
	...
	v, i, _, e := MVCCGet(ctx, engine, key, timestamp, true /* consistent */, txn, nil, false)
	return v, i, e
}
...
func mvccGetInternal(
	...
	if !consistent && txn != nil {
		return nil, nil, safeValue, nil, errors.Errorf(
	...
	if meta.IsInline() {
		...
		*value = roachpb.Value{RawBytes: meta.RawBytes}
		if err := value.Verify(metaKey.Key); err != nil {
			return nil, nil, safeValue, nil, err
		}
		return value, nil, safeValue, nil, nil
	}
	...
	if !timestamp.Less(meta.Timestamp) && meta.Txn != nil && !ownIntent {
		...
		return nil, nil, safeValue, nil, &roachpb.WriteIntentError{
	...
	if !timestamp.Less(meta.Timestamp) || ownIntent {
		...
		if !txn.MaxTimestamp.Less(meta.Timestamp) {
			...
			// one given below.
			return nil, nil, safeValue, nil, roachpb.NewReadWithinUncertaintyIntervalError(
		...
		if seekKey.Timestamp == hlc.ZeroTimestamp {
			return nil, ignoredIntents, safeValue, nil, nil
		}
	}
	...
	return value, ignoredIntents, allowedSafety, wslocks, nil
}
...
func MVCCPut(
	...
	return mvccPutUsingIter(ctx, engine, iter, ms, key, timestamp, value, txn, nil /* valueFn */, slcache, nointent)
}
...
func MVCCBlindPut(
	...
	return mvccPutUsingIter(ctx, engine, nil, ms, key, timestamp, value, txn, nil /* valueFn */, slcache, nointent)
}
...
func MVCCDelete(
	...
	return mvccPutUsingIter(ctx, engine, iter, ms, key, timestamp, noValue, txn, nil /* valueFn */, slcache, nointent)
}
...
func mvccPutUsingIter(
	...
	err := mvccPutInternal(ctx, engine, iter, ms, key, timestamp, rawBytes,
		txn, buf, valueFn, slcache, nointent)
...
func mvccPutInternal(
	...
	if ok {
		...
		} else if !nointent && !meta.Timestamp.Less(timestamp) {
	...
	if timestamp.Less(buf.meta.Timestamp) && nointent {
		if log.V(2) {
			log.Infof(ctx, "Ravi : mvccPutInternal: skipping put, my ts %v , latest ts %v ", timestamp, buf.meta.Timestamp)
		}
...
func MVCCConditionalPut(
	...
	return mvccConditionalPutUsingIter(ctx, engine, iter, ms, key, timestamp, value, expVal, txn, slcache, nointent)
}
...
func MVCCBlindConditionalPut(
	...
	return mvccConditionalPutUsingIter(ctx, engine, nil, ms, key, timestamp, value, expVal, txn, slcache, nointent)
}
...
func MVCCDeleteRange(
	...
	f := func(kv roachpb.KeyValue) (bool, error) {
		...
		if err := mvccPutInternal(ctx, engine, iter, ms, kv.Key, timestamp, nil, txn, buf, nil, slcache, nointent); err != nil {
			return true, err
		}
		if returnKeys {
	...
	_, _, err := MVCCIterate(ctx, engine, key, endKey, hlc.MaxTimestamp, true, txn, false, f, nil, false)
	iter.Close()
	buf.release()
