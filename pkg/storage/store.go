...
func IterateRangeDescriptors(
	...
	_, _, err := engine.MVCCIterate(ctx, eng, start, end, hlc.MaxTimestamp, false /* !consistent */, nil, /* txn */
		false /* !reverse */, kvToDesc, nil, false)
...
func (s *Store) BootstrapRange(initialValues []roachpb.KeyValue) error {
	...
	for _, kv := range initialValues {
		...
		if err := engine.MVCCPut(ctx, batch, ms, kv.Key, now, kv.Value, nil, nil, false); err != nil {
			return err
		}
	}
	// Set range stats.
