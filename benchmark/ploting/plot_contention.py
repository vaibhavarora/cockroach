...
def plot_Tnx_rate():
   ...
   # plt.subplot(211)
    ...
    plt.xticks([w for w in contention_range],['%i:%i'%(50+w,50-w) for w in range(0,50,10)],fontsize = 20)
...
def plot_total_reties():
    ...
    plt.xticks([w for w in contention_range],['%i:%i'%(50+w,50-w) for w in range(0,50,10)],fontsize = 20)
    plt.yticks(np.arange(0, max(max(retries),max(retries1))+1400, 100 ),fontsize = 20)    
